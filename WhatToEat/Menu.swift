...
class Menu:UIView{
    ...
    override init(frame: CGRect) {
        ...
        self.setNeedsLayout()
        self.likeBtn.frame = CGRect(origin: CGPoint(x:0,y:0), size:CGSize(width:80,height:35))
        //self.likeBtn.addTarget(self, action: #selector(Menu.LikeBtn), forControlEvents: .TouchUpInside)
        self.commentBtn.frame = CGRect(origin: CGPoint(x:80,y:0), size:CGSize(width:80,height:35))
    ...
    func menuShow(){
        ...
        UIView.animate(withDuration: 0.2, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: UIViewAnimationOptions.layoutSubviews, animations: {
            self.frame = CGRect(origin: CGPoint(x:self.frame.origin.x - 160,y:self.frame.origin.y), size:CGSize(width:160,height:34))
        }) { (Bool) in
    ...
    func menuHide(){
        ...
        UIView.animate(withDuration: 0.3, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.0, options: UIViewAnimationOptions.layoutSubviews, animations: {
            self.frame = CGRect(origin: CGPoint(x: self.frame.origin.x + 160, y: self.frame.origin.y), size: CGSize(width:0.0, height:34))
        }) { (Bool) in
